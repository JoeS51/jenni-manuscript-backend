...
const uploadFile = async (req, res) => {
    ...
    try {
        ...
        function sendStatus(status, content) {
            res.write(`data: ${JSON.stringify({ status: status, content: content })}\n\n`);
            console.log(status, content);
        }
        // Retrieve AI output
        const journalType = req.body.journalType;
        const manuscriptEvaluationText = await (0, evaluateManuscript_1.evaluateManuscript)(extractedText, journalType, sendStatus);
        res.end();
        ...
        const formattedSectionValidation = manuscriptEvaluationText.sectionValidation; //convertMarkdownToHTML(manuscriptEvaluationText.sectionValidation);
        const formattedGeneralFeedback = manuscriptEvaluationText.generalFeedback; //convertMarkdownToHTML(manuscriptEvaluationText.generalFeedback);
